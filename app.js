...
const $=id=>document.getElementById(id);const show=id=>{document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));if($(id))$(id).classList.add('active');if(window.scrollTo)window.scrollTo(0,0)};
const hideLoader=()=>{const l=document.getElementById('loader');if(l&&!l.classList.contains('done')){l.classList.add('done');setTimeout(()=>{if(l&&l.parentNode)l.style.display='none'},400)}};hideLoader();if(typeof window!=='undefined'){window.addEventListener('load',hideLoader);window.addEventListener('DOMContentLoaded',hideLoader);setTimeout(hideLoader,500);}
function renderPlayers(){const list=$('playerList');list.innerHTML='';players.forEach((name,i)=>{const row=document.createElement('div');row.className='player-row';row.innerHTML=`<span class="player-number">${i+1}</span><input value="${name.replace(/"/g,'&quot;')}" aria-label="Player ${i+1} name"><button class="remove-player" aria-label="Remove player">⌫</button>`;row.querySelector('input').oninput=e=>players[i]=e.target.value||`Player ${i+1}`;row.querySelector('button').onclick=()=>{if(players.length>3){players.splice(i,1);if(imposterCount>Math.floor(players.length/2)){imposterCount=Math.max(1,Math.floor(players.length/2));$('imposterTotal').textContent=imposterCount}renderPlayers()}};list.append(row)});$('playerTotal').textContent=`${players.length} / 10`;}
function renderCategories(){const grid=$('categoryGrid');grid.innerHTML='';packs.forEach(p=>{const b=document.createElement('button');b.className=`category-card ${selected.has(p.id)?'selected':''}`;b.innerHTML=`<span class="cat-icon">${p.icon}</span><b>${p.name}</b><small>${p.words.length} words</small>`;b.onclick=()=>{selected.has(p.id)?selected.delete(p.id):selected.add(p.id);renderCategories();updateCategoryText()};grid.append(b)});$('categoryCount').textContent=`${selected.size} of ${packs.length} selected`;}
function updateCategoryText(){$('categorySummary').textContent=selected.size===packs.length?'All categories selected':`${selected.size} of ${packs.length} selected`;}
...
function startRound(){chooseWord();currentPlayer=0;showPass()};function showPass(){$('passName').textContent=players[currentPlayer]||`Player ${currentPlayer+1}`;if($('roundNumber'))$('roundNumber').textContent=round;show('pass')};
function renderWord(){renderRoleView()}
function showReveal(){showMalayalam=false;renderRoleView();show('reveal')}
function renderRoleView(){const isImposter=imposters.has(currentPlayer);$('roleBadge').textContent=isImposter?(imposters.size>1?'You are one of the Imposters!':'You are the Imposter!'):'You are a player';$('roleBadge').className=`role-badge ${isImposter?'imposter':''}`;$('secretLabel').textContent=isImposter?'YOUR MISSION':'SECRET WORD';if(isImposter){$('secretWord').textContent='?';$('scriptToggle').hidden=!($('showHint').checked&&word&&(word.hintLatin||word.hintMalayalam));$('scriptToggle').textContent=showMalayalam?'Show in Manglish':'Show in Malayalam';let hintHTML=[];if($('seeCategory').checked){hintHTML.push(`<span style="color:#a9a4b3;font-size:11px;text-transform:uppercase;letter-spacing:0.5px;display:block;">Category</span><b style="color:var(--accent);font-size:16px;display:block;margin-top:2px;">${word.category}</b>`)}if($('showHint').checked&&word){const currentHint=showMalayalam?(word.hintMalayalam||word.hintLatin):(word.hintLatin||word.hintMalayalam);if(currentHint){hintHTML.push(`<span style="color:#a9a4b3;font-size:11px;text-transform:uppercase;letter-spacing:0.5px;display:block;">Related Hint</span><b style="color:var(--accent);font-size:16px;display:block;margin-top:2px;">${currentHint}</b>`)}}if(hintHTML.length===0){$('secretHint').innerHTML='Try to work out the secret word from the clues.'}else{$('secretHint').innerHTML=hintHTML.join('<div style="height:10px"></div>')}}else{$('scriptToggle').hidden=false;$('scriptToggle').textContent=showMalayalam?'Show in Manglish':'Show in Malayalam';$('secretWord').textContent=showMalayalam?word.malayalam:word.latin;$('secretHint').textContent='Everyone else has this word too.'}}
function startTimer(){clearInterval(timerId);const minutes=parseInt($('durationSelect')?.value||2,10);let left=minutes*60;const total=left;const draw=()=>{const m=Math.floor(left/60),s=left%60;$('timer').textContent=`${m}:${String(s).padStart(2,'0')}`;if($('timerRing'))$('timerRing').style.strokeDashoffset=327*(1-left/total)};draw();timerId=setInterval(()=>{left--;draw();if(left<=0){clearInterval(timerId);showVote()}},1000)}
function showVote(){clearInterval(timerId);selectedVote=-1;const list=$('voteList');list.innerHTML='';players.forEach((name,i)=>{const b=document.createElement('button');b.className='vote-option';b.innerHTML=`<span class="avatar">${i+1}</span><span>${name}</span>`;b.onclick=()=>{document.querySelectorAll('.vote-option').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');selectedVote=i;$('revealResult').disabled=false;$('revealResult').classList.remove('disabled')};list.append(b)});$('revealResult').disabled=true;$('revealResult').classList.add('disabled');show('vote')}
function result(){const caught=imposters.has(selectedVote);const who=Array.from(imposters).map(i=>players[i]).join(', ');$('resultIcon').className=`result-icon ${caught?'':'fail'}`;$('resultIcon').textContent=caught?'✦':'!';$('resultEyebrow').textContent=caught?'GOT THEM!':'OH NO!';$('resultTitle').textContent=caught?'The Imposter is caught!':(imposters.size>1?'The Imposters win!':'The Imposter wins!');$('resultText').textContent=`${who} ${imposters.size>1?'were the Imposters':'was the Imposter'}.`;$('answerWord').textContent=word.latin;show('result')}
